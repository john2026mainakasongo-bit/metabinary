...
function DesktopTradePage({
  ...
  return (
    <div className="page mbDesktopTradeV81">
      <aside className="mbDeskActivityV81">
        <div className="mbDeskActivityTabsV81"><button className="active">Open {activeBinaryTrade ? "(1)" : "(0)"}</button><button>Closed</button></div>
        ...
        <div className="mbDeskActivityListV81">
          {activeBinaryTrade && (
            <article className="activeTrade">
              ...
              <div><strong>{String(activeBinaryTrade.action || tradeType).toUpperCase()}</strong><small>Stake {money(activeBinaryTrade.stake || safeStake)} USD</small></div>
              <b>{activeBinaryTrade.remainingTicks}t</b>
            </article>
          )}
          {recentTrades.length ? recentTrades.map((item, index) => {
            const won = Boolean(item.won ?? item.result === "win" ?? Number(item.profit || item.pnl || 0) > 0);
            ...
            return (
              <article key={item.id || `${label}-${index}`}>
                <span className={`mbDeskTradeDigitV81 ${won ? "won" : "lost"}`}>{Number.isFinite(resultDigit) ? resultDigit : "•"}</span>
                <div><strong className={won ? "green" : "red"}>{label}</strong><small>Stake {money(item.stake || safeStake)} USD</small></div>
                <b className={won ? "green" : "red"}>{won ? "+" : "-"}{money(Math.abs(amount || Number(item.stake || safeStake)))} USD</b>
              </article>
            );
          }) : !activeBinaryTrade ? (
            <div className="mbDeskEmptyActivityV81"><span>◎</span><strong>No recent trades</strong><small>Your completed trades will appear here.</small></div>
          ) : null}
        </div>
        <button className="mbDeskHistoryBtnV81" type="button" onClick={() => { window.location.hash = "history"; }}>◷ View all history</button>
      </aside>

      <main className="mbDeskCenterV81">
        <section className="mbDeskMarketHeaderV81">
          <div className="mbDeskMarketIdentityV81"><span>{binaryMarket?.short || "V50"}</span><div><small>VOLATILITY MARKET</small><strong>{binaryMarket?.label || "Volatility 50 Index"}</strong></div><i>● LIVE</i></div>
        ...
        <section className="mbDeskChartPanelV81">
          <div className="mbDeskChartToolbarV81"><button>1S⌄</button><button>⌁</button><button>▥</button><button>Indicators⌄</button><button>↶</button><button>↷</button></div>
          <div className="mbDeskChartCanvasV81">
            <LineChart data={prices.map((value) => value * Number(binaryMarket?.scale || 800))} anchorValue={activeBinaryTrade && tradeType === "Rise/Fall" ? activeTradeEntry * Number(binaryMarket?.scale || 800) : null} />
            <div className="mbDeskChartPriceV81">{indexValue.toFixed(2)}</div>
            <div className="mbDeskChartAxisV81"><span>{(indexValue + priceStep * 2).toFixed(2)}</span><span>{(indexValue + priceStep).toFixed(2)}</span><span>{indexValue.toFixed(2)}</span><span>{(indexValue - priceStep).toFixed(2)}</span><span>{(indexValue - priceStep * 2).toFixed(2)}</span></div>
            {activeBinaryTrade && <div className="mbDeskActiveTradeBadgeV81"><i></i><strong>{activeBinaryTrade.action}</strong><small>{activeBinaryTrade.remainingTicks} ticks remaining</small></div>}
          </div>
          <div className="mbDeskTimeAxisV81"><span>18:38:05</span><span>18:38:20</span><span>18:38:35</span><span>18:38:50</span><span>18:39:05</span><span>18:39:20</span><span>18:39:35</span><span>18:39:50</span></div>
          <div className="mbDeskDigitsV81">
            {digitStats.map((percent, digit) => {
              ...
              return (
                <button key={digit} type="button" disabled={Boolean(activeBinaryTrade)} onClick={() => setPrediction(digit)} className={[isHighest ? "highest" : "", isLowest ? "lowest" : "", isPicked ? "picked" : "", isCurrent ? "current" : "", isResult ? binaryResultFlash?.result : ""].filter(Boolean).join(" ")}>
      ...
      <aside className="mbDeskTradePanelV81">
        ...
        <div className="mbDeskTradeTabsV81">
          {["Even/Odd", "Matches/Differs", "Over/Under", "Rise/Fall"].map((type) => <button key={type} className={tradeType === type ? "active" : ""} onClick={() => setTradeType(type)} disabled={Boolean(activeBinaryTrade)}>{type}</button>)}
        </div>
        <label className="mbDeskMarketSelectV81"><span>{binaryMarket?.short || "V50"}</span><select value={binaryMarketId} onChange={(event) => setBinaryMarketId(event.target.value)} disabled={Boolean(activeBinaryTrade)}>{volatilityOptions.map((market) => <option key={market.id} value={market.id}>{market.label}</option>)}</select><i>● LIVE</i></label>
        <div className="mbDeskStakeHeadV81"><span>Stake Amount</span><small>(USD)</small></div>
        <div className="mbDeskStakeControlV81"><button onClick={() => changeStake(-1)} disabled={Boolean(activeBinaryTrade)}>−</button><input type="number" min="0.3" step="0.1" value={stake} onChange={(event) => setStake(event.target.value === "" ? "" : Number(event.target.value))} disabled={Boolean(activeBinaryTrade)} /><button onClick={() => changeStake(1)} disabled={Boolean(activeBinaryTrade)}>+</button></div>
        <div className="mbDeskQuickStakeV81">{quickStakes.map((value) => <button key={value} className={Number(stake) === value ? "active" : ""} onClick={() => setStake(value)} disabled={Boolean(activeBinaryTrade)}>{value}</button>)}</div>
        <div className="mbDeskDurationV81"><span>Duration</span><div><button onClick={() => setDuration(Math.max(1, Number(duration) - 1))} disabled={Boolean(activeBinaryTrade)}>−</button><strong>{duration} tick{Number(duration) === 1 ? "" : "s"}</strong><button onClick={() => setDuration(Math.min(10, Number(duration) + 1))} disabled={Boolean(activeBinaryTrade)}>+</button></div></div>
        <div className="mbDeskPayoutLineV81"><span>Estimated payout</span><strong>{Math.max(leftRate, rightRate).toFixed(2)}×</strong></div>
        <button className="mbDeskActionV81 even" onClick={() => placeContract(rightAction)} disabled={Boolean(activeBinaryTrade) || rightRate <= 0}><strong>{rightLabel}</strong><span>Payout {rightPayout} USD</span><small>Profit {rightRate > 0 ? money(safeStake * rightRate - safeStake) : "—"} USD</small></button>
        <button className="mbDeskActionV81 odd" onClick={() => placeContract(leftAction)} disabled={Boolean(activeBinaryTrade) || leftRate <= 0}><strong>{leftLabel}</strong><span>Payout {leftPayout} USD</span><small>Profit {leftRate > 0 ? money(safeStake * leftRate - safeStake) : "—"} USD</small></button>
        <button className="mbDeskAiCardV81" type="button" onClick={openAiScanner}><span>AI</span><div><strong>AI Scanner</strong><small>Analyze market & get smart insights</small></div><b>Scan Now</b></button>
      </aside>

      <footer className="mbDeskFooterV81"><span><i></i> Connected</span><small>Server Time: {new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })} (UTC+3)</small><strong>◈ Secured by MetaBinary</strong><button onClick={() => { window.location.hash = "settings"; }}>⚙</button></footer>
    </div>
  );
}
